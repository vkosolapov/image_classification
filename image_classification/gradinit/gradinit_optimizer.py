    ...
    def step(self, closure=None, is_constraint=False):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                grad = (
                    torch.sum(p.grad * p.data).item() * state["init_norm"] / curr_norm
                )
                ...
                if is_constraint:
                    state["cons_step"] += 1
                    state["cons_exp_avg"] = state["cons_exp_avg"] * beta1 + grad * (
                        1 - beta1
                    )
                ...
                state["exp_avg_sq"] = state["exp_avg_sq"] * beta2 + (grad * grad) * (
                    1 - beta2
                )
                ...
                state["alpha"] = max(
                    state["alpha"] - step_size * exp_avg / denom, group["min_scale"]
                )
