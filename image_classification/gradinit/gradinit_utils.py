...
from .gradinit_optimizer import RescaleAdam
from .gradinit_modules import Scale, Bias
import numpy as np
import os
...
def gradinit(net, dataloader, args=default_args):
        ...
        [
            {
                ...
                "min_scale": args["gradinit_min_scale"],
                "lr": args["gradinit_lr"],
            },
    ...
    while True:
        ...
        all_grads = torch.autograd.grad(init_loss, params_list, create_graph=False)
        ...
        if gnorm.item() > args["gradinit_gamma"]:
            ...
            if args["batch_no_overlap"]:
                ...
                data_iter, init_inputs_0, init_targets_0 = get_batch(
        ...
        total_iters += 1
        if (
            (total_sums_gnorm > 0 and total_sums_gnorm % 10 == 0)
            or total_iters == args["gradinit_iters"]
            or total_iters == args["gradinit_iters"]
        ):
            stat_dict = get_scale_stats(net, optimizer)
            print_str = (
                "Iter {}, obj iters {}, eta {:.3e}, constraint count {} loss: {:.3e} ({:.3e}), init loss: {:.3e} ({:.3e}), update loss {:.3e} ({:.3e}), "
    ...
    torch.save(net.state_dict(), "checkpoints/{}_init.pth".format(args["expname"]))


def gradient_quotient(loss, params, eps=1e-5):
    grad = torch.autograd.grad(loss, params, create_graph=True)
    prod = torch.autograd.grad(
        sum([(g ** 2).sum() / 2 for g in grad]), params, create_graph=True
    )
    out = sum(
        [
            ((g - p) / (g + eps * (2 * (g >= 0).float() - 1).detach()) - 1).abs().sum()
...
def metainit(net, dataloader, args=default_args, experiment=None):
    ...
    for ite, (inputs, targets) in enumerate(dataloader):
        ...
        if (
            ite % 10 == 0
            or ite == args["gradinit_iters"] - 1
            or ite == len(dataloader) - 1
        ):
            stat_dict = get_scale_stats(net, optimizer)
